...
class Paginator {
    ...
    public function paginate() {
        ...
        $current_page = (int)$this->active_page; // must be numeric > 0
        ...
        if($total_pages > $this->items_per_page) {
            $output .= ($current_page != 1 && $this->items_total >= $this->items_per_page) 
                ? '<li><a class="paginate '.$this->link_class.' prev" href="' . $base_uri . $prev_page . '">«</a></li>'
            ...
            for($i = 1; $i <= $total_pages; $i++) {
                ...
                if($i == 1 || $i == $total_pages || in_array($i, $this->range)) {
                    $output .= ($i == $current_page) 
                        ...
                        : '<li><a class="'.$this->link_class.'" title="Go to page '.$i.' of '.$total_pages.'" href="'.$base_uri.$i.'">'.$i.'</a></li>';
                }
            ...
            $output .= (($current_page != $total_pages && $this->items_total >= $this->items_per_page) && (@$_GET[$this->page_name] != 'All')) 
                ? '<li><a class="'.$this->link_class.' next" href="'.$base_uri.$next_page.'">»</a></li>'
        ...
        else
        {
            ...
            for($i=1;$i<=$total_pages;$i++) {
                $output .= ($i == $current_page) 
                    ...
                    : '<li><a class="'.$this->link_class.'" href="'.$base_uri.$i.'">'.$i.'</a></li>';
            }
